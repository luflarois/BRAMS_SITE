  ...
  <section id="main-timeline">
    <div class="container">
      <div id="tline-content">
		...
		?>
		<article id="post-<?php the_ID(); ?>"  class="tline-box <?php if(($i%2)==0 && $flag) { $flag = false; $i++; } elseif( ($i%2)==0 ) echo ' rgtline'; ?>"> <span class="tline-row-<?php if(($i%2)==0) echo 'r'; else echo 'l'; ?>"></span> <?php
	  ...
	  <?php } } } ?>
			...
			<h6 class="blog-author"><?php _e('at','WEBNUS_TEXT_DOMAIN'); ?> <?php the_category(', ') ?>
			<?php endif; ?>
			<?php if(  $webnus_options->webnus_blog_meta_category_enable() && $webnus_options->webnus_blog_meta_author_enable()):?>
			 -
			<?php endif; ?>
			<?php if(  $webnus_options->webnus_blog_meta_author_enable() ):?> 
			<?php _e('by','WEBNUS_TEXT_DOMAIN'); ?> <?php the_author(); ?></h6>
			<?php endif; ?>
          </div><p>
          <?php 
